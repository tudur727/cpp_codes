...
///what's new? (1.7)
/// - windows support? (merge shit, dar merge...)
/// trebuie sa fac sa dea refresh frmos...
...
void userCreateShape(){
    deltax = deltay = deltaz = 0;
    shape userCreated("empty");
    while(true){
        clearScreen();
        int key = getKey();
        if( key == 'q')
...
int main(){
    shape engine("cub"); // de aici alegi ce forma da render
    int VERTEXES = engine.getSize();
    bool autospin = true;
    Vec3 t; // auxiliare
    Vec2 p; // auxiliare
    ...
    while(true){
        ...
        if (autospin){
                deltax += engine.speedxaxis/1000;
                deltay += engine.speedyaxis/1000;
                deltaz += engine.speedzaxis/1000;
        }else{
            if(key == 'd')
                deltay -= 0.15;
            if (key == 'a')
        ...
        if(key=='r'){
            engine.cycleType();
            VERTEXES = engine.getSize();
            /// MEREU SA DAI UPDATE LA SIZE CAND SCHIMBI CEVA...
        }
        if (key == ']')
            engine.CameraDistanceModifier(0.5);
        if (key == '[')
            engine.CameraDistanceModifier(-0.5);
        if (key == 'm')
            manual();
        ...
        for(int i=0; i<VERTEXES; i++){
            t = engine.transformpoint(engine.getPoint(i));
            t.y -= engine.offsety;
            t.y += engine.offsetx;
            t.z += engine.offsetz;
            p = engine.projectedpoint(t);
            engine.setTemp(i,t);
            engine.setProjected(i,p);
            //std::cout<<"MARIAN \n";
        }
        engine.render();       // -> iau punctele si trasez linii intre ele
        std::cout<<"shape: "<<engine.getType()<<"\n";
        std::cout<<"autospin: "<<autospin<<"\n";
        std::cout<<"distance: "<<engine.GetCameraDistance()<<"\n";
